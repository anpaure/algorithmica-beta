...
In the context of hardware, such a scheme is called a *fully associative cache*: we have $M$ cells, each capable of holding a cache line corresponding to any of the $N$ total memory locations, and in case of contention, the one not accessed the longest gets kicked out and replaced with the new one.
...
If we implemented a set-associative cache in software, we would compute some hash function of the memory block address and then use its value as the cache line index. In hardware, we can't really do that because it is too slow: for example, for the L1 cache, the latency requirement is 4 or 5 cycles, and even [taking a modulo](/hpc/arithmetic/division) takes around 10-15 cycles, let alone something more sophisticated.
...
- *index* — the index of the cache line set (the next $12$ bits as there are $2^{12}$ sets in the L3 cache);
...
On every access, the cache compares the requested tag with all ways of the selected set. A hit chooses the matching way; a miss chooses one way to replace. For a 2-way set, exact LRU needs only one state bit: a hit in way 0 marks way 1 as the next victim and vice versa.

Exact LRU becomes more expensive as associativity grows because the hardware has to maintain the relative order of all ways. Real processors often use a cheaper pseudo-LRU or another undocumented approximation. This changes which line loses a close contest, but not the pathology above: once more than 16 simultaneously live lines map to a 16-way set, some of them must be evicted regardless of the replacement rule. The [software implementation](../../external-memory/policies/#implementing-caching) uses an explicit linked list because it does not have the same latency and transistor constraints.
